...
class Step1Form(StepLocationForm):
    pass
...
class Step3Form(StepIncomeForm):
    pass
...
class Step6Form(StepPracticeAreaCategoryForm):
    def __init__(self, *args, **kwargs):
        ...
        for practice in models.PracticeAreaCategory.objects.filter(practicearea_category_type=ContentType.objects.get_for_model(Lawyer)).extra(select={'pid': 'CAST(id AS INTEGER)'}).order_by('pid'):
...
class BaseReferralStepForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        allow_llm_field = self.fields.get('allow_llm')
        if allow_llm_field:
            self.initial["allow_llm"] = "true"
        ...
        if professional_field:
            ...
            professionals = utils.filterLawyers(practice_area=practice_area)
    ...
    def clean(self):
        ...
        if practice_area and not Lawyer.objects.get_lawyer_matches(practice_area).count():
                raise ValidationError(_('We are sorry, there are no lawyers in the LowBono network who match your search. Please try searching again or look at our resources page for other resources.'))
    ...
    class Meta:
        ...
        widgets = {
            ...
            'professional': widgets.RadioLawyerDetailSelect,
            'email': forms.TextInput(attrs={'type': 'email'}),
            'address': forms.Textarea(attrs={'rows': '3'}),
            'allow_llm': forms.HiddenInput,
        }


class Step11Form(BaseReferralStepForm):
    # override default so we don't get ------ as first of three choicese.
    contact_preference = forms.ChoiceField(label=constants.CONTACT_REQUEST_LABEL_LAWYER, widget=forms.RadioSelect, choices=constants.CONTACT_REQUEST_CHOICES)
    ...
    class Meta(BaseReferralStepForm.Meta):
        fields = ['first_name', 'last_name', 'email', 'phone', 'language', 'address', 'contact_preference', 'follow_up_consent', 'referred_by']
