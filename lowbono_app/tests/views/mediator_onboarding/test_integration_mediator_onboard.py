...
class MediatorOnboardIntegrationTest(LiveServerTestCase):
    ...
    def test_mediator_onboard(self):
        ...
        self.browser.fill('email', 'a2@lowbono.org')
        self.browser.fill('first_name', 'test')
        self.browser.fill('last_name', 'user')
        self.browser.find_by_css('input[type=radio][value=mediator]').first.click()
        self.browser.find_by_css('button[type=submit]').first.click()
        ...
        actual = 'a2@lowbono.org'
        expected = mail.outbox[0].to
        self.assertIn(actual, expected)

        self.attorney_user = User.objects.filter(email='a2@lowbono.org').first()

        # Mediator user opens the link sent in the email and sees new password page
        ...
        # Mediator user setup password on the signup page
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()
        self.assertIn("Account created! Please login to access profile", self.browser.html)

        # Mediator user go to /login page and enter details
        self.browser.visit(self.login_url)
        self.browser.fill('email', 'a2@lowbono.org')
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()

        # Mediator user first login redirects to profile edit page
        self.assertIn("Edit Basic Details", self.browser.html)

        # Mediator user visits profile details page and sees a warning
        ...
        # Mediator user goes to profile edit page and fill all columns and save
        ...
        self.browser.fill('email', 'A2@LOWBONO.com')
        ...
        self.browser.find_by_css('input[name=mediator_user-0-practice_areas]').first.check()
        ...
        self.assertNotIn("A2@LOWBONO.com", self.browser.html)
        self.assertIn("a2@lowbono.org", self.browser.html)
        self.browser.visit(self.logout_url)

        # Mediator user logs in again and gets redirected to referrals list page
        self.browser.visit(self.login_url)
        self.browser.fill('email', 'a2@lowbono.org')
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()
