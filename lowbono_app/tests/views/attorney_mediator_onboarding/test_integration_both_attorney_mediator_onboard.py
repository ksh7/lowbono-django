...
class AttorneyAndMediatorOnboardIntegrationTest(LiveServerTestCase):
    ...
    def test_both_attorney_mediator_onboard(self):
        ...
        self.browser.fill('email', 'a3@lowbono.org')
        self.browser.fill('first_name', 'test')
        self.browser.fill('last_name', 'user')
        self.browser.find_by_css('input[type=radio][value=both]').first.click()
        self.browser.find_by_css('button[type=submit]').first.click()
        ...
        actual = 'a3@lowbono.org'
        expected = mail.outbox[0].to
        self.assertIn(actual, expected)

        self.attorney_user = User.objects.filter(email='a3@lowbono.org').first()

        # User opens the link sent in the email and sees new password page
        ...
        # User setup password on the signup page
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()
        self.assertIn("Account created! Please login to access profile", self.browser.html)

        # User go to /login page and enter details
        self.browser.visit(self.login_url)
        self.browser.fill('email', 'a3@lowbono.org')
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()

        # User first login redirects to profile edit page
        self.assertIn("Edit Basic Details", self.browser.html)

        # User visits profile details page and sees a warning
        ...
        # User goes to profile edit page and fill all columns and save
        ...
        self.browser.fill('email', 'A3@LOWBONO.com')
        ...
        self.assertNotIn("A3@LOWBONO.com", self.browser.html)
        self.assertIn("a3@lowbono.org", self.browser.html)
        self.browser.visit(self.logout_url)

        # User logs in again and gets redirected to referrals list page
        self.browser.visit(self.login_url)
        self.browser.fill('email', 'a3@lowbono.org')
        self.browser.fill('password', 'newtestpassword')
        self.browser.find_by_css('button[type=submit]').first.click()
